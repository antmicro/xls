...
proc BlockDecoderTest {
    ...
    next(tok: token, state: ()) {
        ...
        let tok = for ((counter, block_packet), tok): ((u32, BlockDataPacket), token) in
            enumerate(EncodedDataBlocksPackets) {
        ...
        let tok = for ((counter, expected_block_packet), tok): ((u32, BlockData), token) in
            enumerate(DecodedDataBlocksPackets) {
