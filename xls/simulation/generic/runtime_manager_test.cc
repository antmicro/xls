 ...
 private:
  std::vector<Value> create_increasing_input_values(int amount, int bit_count,
    ...
    for (int i = start; i < start + amount; i = i + increment) {
      data_.push_back(Value(UBits(i, bit_count)));
    }
    return data_;
  }
 ...
 public:
  void SetUp() override {
    XLS_ASSERT_OK_AND_ASSIGN(std::filesystem::path ir_path,
                             xls::GetXlsRunfilePath(ir_file_));
    XLS_ASSERT_OK_AND_ASSIGN(std::string ir_text,
                             xls::GetFileContents(ir_path));
    XLS_ASSERT_OK_AND_ASSIGN(package_, xls::Parser::ParsePackage(ir_text));
    data_vector_ = create_increasing_input_values(10, 32);
  }

  std::unique_ptr<Package> package_;
  std::unique_ptr<RuntimeManager> manager_;
  std::vector<Value> data_vector_;
  std::string recv_channel_name_ = "passthrough__data_s";
  std::string send_channel_name_ = "passthrough__data_r";
  std::uint8_t status_addr_ = 0x0;
  std::string ir_file_ =
      "xls/simulation/generic/testdata/pasthrough_opt_ir.opt.ir";
};

TEST_P(RuntimeManagerTest, Init) {
  XLS_EXPECT_OK(RuntimeManager::Create(package_.get(), /*use_jit=*/GetParam()));
}

TEST_P(RuntimeManagerTest, DeadlockStateAfterInit) {
  XLS_ASSERT_OK_AND_ASSIGN(
      auto manager,
      RuntimeManager::Create(package_.get(), /*use_jit=*/GetParam()));
...
TEST_P(RuntimeManagerTest, RuntimeSimulation) {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(auto recv_queue,
                           queue_manager.GetQueueByName(recv_channel_name_));
  XLS_ASSERT_OK_AND_ASSIGN(auto send_queue,
                           queue_manager.GetQueueByName(send_channel_name_));
  XLS_EXPECT_OK(manager->Reset());

  for (const auto& x : data_vector_) {
  ...
  for (int i = 0; i < data_vector_.size(); ++i) {
    ...
    EXPECT_EQ(val.value(), data_vector_[i]);
  }
}
...
TEST_P(RuntimeManagerTest, GetPayloadData) {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(auto value8, status.GetPayloadData8(status_addr_));
  EXPECT_EQ(value8, static_cast<uint8_t>(0x0));
  XLS_ASSERT_OK_AND_ASSIGN(auto value16, status.GetPayloadData16(status_addr_));
  EXPECT_EQ(value16, static_cast<uint16_t>(0x0));
  XLS_ASSERT_OK_AND_ASSIGN(auto value32, status.GetPayloadData32(status_addr_));
  EXPECT_EQ(value32, static_cast<uint32_t>(0x0));
  XLS_ASSERT_OK_AND_ASSIGN(auto value64, status.GetPayloadData64(status_addr_));
  EXPECT_EQ(value64, static_cast<uint64_t>(0x0));

  EXPECT_EQ(manager->Update().code(), absl::StatusCode::kInternal);
  EXPECT_EQ(manager->HasDeadlock(), true);

  XLS_ASSERT_OK_AND_ASSIGN(value8, status.GetPayloadData8(status_addr_));
  EXPECT_EQ(value8, static_cast<uint8_t>(0x1));
  XLS_ASSERT_OK_AND_ASSIGN(value16, status.GetPayloadData16(status_addr_));
  EXPECT_EQ(value16, static_cast<uint16_t>(0x1));
  XLS_ASSERT_OK_AND_ASSIGN(value32, status.GetPayloadData32(status_addr_));
  EXPECT_EQ(value32, static_cast<uint32_t>(0x1));
  XLS_ASSERT_OK_AND_ASSIGN(value64, status.GetPayloadData64(status_addr_));
