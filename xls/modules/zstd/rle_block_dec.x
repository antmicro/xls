...
// * RunLengthDecoder
// * BatchPacker
// Connections between those is represented on the diagram bellow:
//
//                                RleBlockDecoder
//    ┌─────────────────────────────────────────────────────────────┐
...
//    └─────────────────────────────────────────────────────────────┘
//
// RleDataPacker is responsible for receiving the incomming packets of block data, converting
...
struct BlockDataSynchronization {
    last_block: bool,
    id: u32
}
...
proc RleDataPacker {
    ...
    sync_s: chan<BlockDataSynchronization> out;
    ...
    next(tok: token, state: ()) {
        ...
        let sync_data = BlockDataSynchronization { last_block: input.last_block, id: input.id };
...
proc RleDataPacker_test {
    ...
    sync_r: chan<BlockDataSynchronization> in;

    config(terminator: chan<bool> out) {
        let (in_s, in_r) = chan<BlockDataPacket>;
        let (out_s, out_r) = chan<RleInput>;
        let (sync_s, sync_r) = chan<BlockDataSynchronization>;
    ...
    next(tok: token, state: ()) {
        ...
        let tok = for ((counter, block), tok): ((u32, RleTestVector), token) in
            enumerate(EncodedRleBlocks) {
            ...
            let sync_out = BlockDataSynchronization {
                id: counter,
...
proc BatchPacker {
    ...
    sync_r: chan<BlockDataSynchronization> in;
    block_data_s: chan<BlockDataPacket> out;
    ...
    next(tok: token, state: BatchState) {
        ...
        let block_in_progress_sync =
            BlockDataSynchronization { id: state.prev_id, last_block: state.prev_last_block };
        let (tok, sync_data) =
            recv_if(tok, sync_r, state.prev_last, block_in_progress_sync);

        let decoded_batch_data = BlockDataPacket {
            last: decoded_data.last,
            last_block: sync_data.last_block,
            id: sync_data.id,
            data: batch,
            length: symbols_in_batch
        };
...
proc BatchPacker_test {
    ...
    sync_s: chan<BlockDataSynchronization> out;
    out_r: chan<BlockDataPacket> in;

    config(terminator: chan<bool> out) {
        let (in_s, in_r) = chan<RleOutput>;
        let (sync_s, sync_r) = chan<BlockDataSynchronization>;
        let (out_s, out_r) = chan<BlockDataPacket>;
    ...
    next(tok: token, state: ()) {
        let SyncData: BlockDataSynchronization[6] = [
            BlockDataSynchronization { last_block: false, id: u32:0 },
            BlockDataSynchronization { last_block: false, id: u32:1 },
            BlockDataSynchronization { last_block: false, id: u32:2 },
            BlockDataSynchronization { last_block: false, id: u32:3 },
            BlockDataSynchronization { last_block: false, id: u32:4 },
            BlockDataSynchronization { last_block: true, id: u32:5 },
        ];
        let tok = for ((counter, sync_data), tok): ((u32, BlockDataSynchronization), token) in
            enumerate(SyncData) {
        ...
        let tok = for ((counter, test_data), tok): ((u32, BatchTestVector), token) in
            enumerate(DecodedRleBlocks) {
        ...
        let BatchedDecodedRleSymbols: BlockDataPacket[10] = [
            BlockDataPacket {last: bool:true, last_block: bool:false, id: u32:0, data: u64:0x01, length: 1},
            BlockDataPacket {last: bool:true, last_block: bool:false, id: u32:1, data: u64:0x0202, length: 2},
            BlockDataPacket {last: bool:true, last_block: bool:false, id: u32:2, data: u64:0x03030303, length: 4},
            BlockDataPacket {last: bool:true, last_block: bool:false, id: u32:3, data: u64:0x0404040404040404, length: 8},
            BlockDataPacket {last: bool:false, last_block: bool:false, id: u32:4, data: u64:0x0505050505050505, length: 8},
            BlockDataPacket {last: bool:true, last_block: bool:false, id: u32:4, data: u64:0x0505050505050505, length: 8},
            BlockDataPacket {last: bool:false, last_block: bool:true, id: u32:5, data: u64:0x0606060606060606, length: 8},
            BlockDataPacket {last: bool:false, last_block: bool:true, id: u32:5, data: u64:0x0606060606060606, length: 8},
            BlockDataPacket {last: bool:false, last_block: bool:true, id: u32:5, data: u64:0x0606060606060606, length: 8},
            BlockDataPacket {last: bool:true, last_block: bool:true, id: u32:5, data: u64:0x06060606060606, length: 7},
        ];

        let tok = for ((counter, expected), tok): ((u32, BlockDataPacket), token) in
            enumerate(BatchedDecodedRleSymbols) {
            let data_out = BlockDataPacket {
                last: expected.last,
                last_block: expected.last_block,
                id: expected.id,
                data: expected.data,
...
pub proc RleBlockDecoder {
    ...
    config(input_r: chan<BlockDataPacket> in, output_s: chan<BlockDataPacket> out) {
