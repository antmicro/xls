...
proc ConditionalIoTest {
    ...
    // state=0 fires the single test iteration; subsequent iterations are no-ops
    // so SPIN can run extra loop iterations before __terminated is set without
    // producing spurious channel events that would diverge from the DSLX trace.
    init { u32:0 }
    next(state: u32) {
        let tok = send_if(join(), recv_if_true_s, state == u32:0, u32:42);
        send_if(tok, terminator, state == u32:0, true);
        state + u32:1
    }
}
