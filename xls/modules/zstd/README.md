...
The Decoder interacts with the environment through sets of ports:
* Memory Interface
* CSR Interface
* Notify line
* Stream Output

The software controlls the core through a set of registers which are used
to configure the decoder and to start the decoding process.

ZSTD frames to decode are placed in a memory that should be connected to
decoder's memory interface port.
...
7. (Optional) Calculates checksum and compares it against frame checksum.

![](img/ZSTD_decoder.png)

## Registers description

ZSTD Decoder operation bases on the values stored in a set of CSRs accessible for the user through common AXI bus.
The registers are defined below:

| Name | Access | Address | Description |
| ---- | ------ | ------- | ----------- |
| Status | RO | 0x0 | Keeps the code describing the current state of the ZSTD Decoder |
| Start | RW | 0x4 | Writing `1` when decoder is in `IDLE` state starts the decoding process |
| Reset | RW | 0x8 | Writing `1` will reset the decoder to the `IDLE` state |
| Input Buffer | RW | 0xC | Keeps the base address for the input buffer that is used for storing the frame to decode |
| Output Buffer | RW | 0x10 | Keeps the base address for the output buffer, ZSTD Decoder will write the decoded frame into memory starting from this address. |
| Who Am I | RO | 0x3C | Contains the identification number of the ZSTD Decoder |
...
It uses the configuratin data from the CSRs, connects all underlying modules and sends processing requests to those based on the state of the machine.
...
Cocotb testbench interacts with the decoder with the help of a [cocotbext-axi](https://github.com/alexforencich/cocotbext-axi) extension that provides AXI bus models, drivers, monitors and RAM model accessible through AXI interface.
