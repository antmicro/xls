...
// Entry point for the XLS->Promela code generator.
//
// Reads an XLS IR file and emits Promela source code suitable for model
// checking with the SPIN verifier (https://spinroot.com).
//
// Usage:
//   promela_main [--output=PATH] IR_FILE
//
// If --output is omitted the Promela source is written to stdout.
...
Example:
  promela_main my_design.ir
  promela_main --output=model.pml my_design.ir
)";
}  // namespace
...
absl::Status RealMain(std::string_view ir_path) {
  ...
  const std::string thr_flag = absl::GetFlag(FLAGS_worst_case_throughput);
  if (!thr_flag.empty()) {
    for (std::string_view entry : absl::StrSplit(thr_flag, ',')) {
      ...
      int n;
      if (!absl::SimpleAtoi(parts[1], &n) || n < 1) {
        return absl::InvalidArgumentError(absl::StrFormat(
            "--worst_case_throughput: N must be a positive integer, got '%s'",
            parts[1]));
      }
      options.worst_case_throughput[std::string(parts[0])] = n;
    }
  }

  XLS_ASSIGN_OR_RETURN(std::string promela_text,
