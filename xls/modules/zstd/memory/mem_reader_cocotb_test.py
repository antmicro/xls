...
from cocotbext.axi.axi_channels import AxiARBus, AxiRBus, AxiReadBus
...
def generate_test_data(test_cases, xfer_base=0x0, seed=1234):
  ...
  ctrl = [MemReaderCtrl(base=xfer_base)]
  req = []
  resp = []
  mem_writes = {}
  ...
  return (ctrl, req, resp, mem_writes)


async def test_mem_reader(dut, ctrl_input, req_input, resp_output, mem_contents={}):
  ...
  mem_reader_ctrl_driver = XLSChannelDriver(dut, MEM_READER_CTRL_CHANNEL, dut.clk)
  ...
  await mem_reader_ctrl_driver.send(ctrl_input)
...
async def mem_reader_zero_length_req(dut):
  ctrl, req, resp, _ = generate_test_data(
    xfer_base=0xFFF, test_cases=[(0x101, 0)]
  )
  await test_mem_reader(dut, ctrl, req, resp)


@cocotb.test(timeout_time=500, timeout_unit="ms")
async def mem_reader_aligned_transfer_shorter_than_bus(dut):
  ctrl, req, resp, mem_contents = generate_test_data(
    xfer_base=0xFFF, test_cases=[(0x101, 1)]
  )
  await test_mem_reader(dut, ctrl, req, resp, mem_contents)
...
if __name__ == "__main__":
  ...
  verilog_sources = [
    "xls/modules/zstd/memory/mem_reader.v",
