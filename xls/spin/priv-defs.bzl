...
def _spin_trace_test_impl(ctx):
    ...
    ctx.actions.write(
        ...
        content = "\n".join([
            "#!/usr/bin/env bash",
            "set -e",
            "{compare} --spin_trace={spin} --dslx_trace={dslx}".format(
    ...
    runfiles = ctx.runfiles(files = [compare, spin_json, dslx_json])
...
differently; only the relative order within each channel is checked.
  ...
  spin_trace_test(
      ...
      spin_trace = ":my_spin_trace",  # spin_trace
      dslx_trace = ":my_dslx_trace",  # dslx_trace
  )
