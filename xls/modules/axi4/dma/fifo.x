...
import xls.modules.axi4.dma.gpf
import xls.modules.axi4.dma.axi_st

type AXI_ST = axi_st::AXI_ST;

pub struct ReaderState<ADDR_W: u32> {
    ptr_r: uN[ADDR_W],
    is_empty: u1,
    ptr_w: uN[ADDR_W],
}

pub struct WriterState<ADDR_W: u32> { ptr_w: uN[ADDR_W], is_full: u1, ptr_r: uN[ADDR_W] }
...
proc Reader<DATA_W: u32, DATA_W_DIV8: u32, ID_W: u32, DEST_W: u32, ADDR_W: u32, FIFO_LENGTH: u32> {
    ch_read: chan<AXI_ST<DATA_W, DATA_W_DIV8, ID_W, DEST_W>> out;
    ...
    config(ch_read: chan<AXI_ST<DATA_W, DATA_W_DIV8, ID_W, DEST_W>> out,
        ch_mem_read_req:chan<ram::RWRamReq<ADDR_W, DATA_W, u32:0>> out,
    ch_mem_read_rsp: chan<ram::RWRamResp<DATA_W>> in, ch_ptr_r: chan<ReaderState<ADDR_W>> out, ch_ptr_w
    :
    chan<WriterState<ADDR_W>> in) {
        (ch_read, ch_mem_read_req, ch_mem_read_rsp, ch_ptr_r, ch_ptr_w)
    }

    init {
        (ReaderState<ADDR_W> {
            ptr_r: uN[ADDR_W]:0, is_empty: u1:1, ptr_w: uN[ADDR_W]:0,
        })
    }
    ...
    next(tok: token, state: ReaderState<ADDR_W>) {
        ...
        let zero_rw_ram_resp = ram::RWRamResp { data: uN[DATA_W]:0 };
        let (tok, read_data) = recv_if(tok, ch_mem_read_rsp, !is_empty, zero_rw_ram_resp);

        // Push data into stream interface
        let tok = send_if(
            tok, ch_read, !is_empty,
            AXI_ST {
        ...
        let ptr_r = if !is_empty { state.ptr_r + uN[ADDR_W]:1 } else { state.ptr_r };
...
proc Writer<DATA_W: u32, DATA_W_DIV8: u32, ID_W: u32, DEST_W: u32, ADDR_W: u32, FIFO_LENGTH: u32> {
    ch_write: chan<AXI_ST<DATA_W, DATA_W_DIV8, ID_W, DEST_W>> in;
    ...
    config(ch_write: chan<AXI_ST<DATA_W, DATA_W_DIV8, ID_W, DEST_W>> in, ch_mem_write_req
    :
    chan<ram::RWRamReq<ADDR_W, DATA_W, u32:0>> out,
    ch_mem_write_rsp: chan<()> in, ch_ptr_r: chan<ReaderState<ADDR_W>> in, ch_ptr_w
    :
    chan<WriterState<ADDR_W>> out) {
    ...
    next(tok: token, state: WriterState<ADDR_W>) {
        ...
        let (tok, reader_state, update_ptr_r) = recv_non_blocking(tok, ch_ptr_r, zero!<ReaderState>());
...
#[test_proc]
proc test_fifo {
    ch_fifo_read: chan<AXI_ST<TEST_0_DATA_W, TEST_0_DATA_W_DIV8, TEST_0_ID_W, TEST_0_DEST_W>> in;
    ch_fifo_write: chan<AXI_ST<TEST_0_DATA_W, TEST_0_DATA_W_DIV8, TEST_0_ID_W, TEST_0_DEST_W>> out;
    ...
    next(tok: token, state: TestState<TEST_0_DATA_W>) {
        // Write to FIFO
        let write_data = AXI_ST<TEST_0_DATA_W, TEST_0_DATA_W_DIV8, TEST_0_ID_W, TEST_0_DEST_W> {
            tdata: state.data,
        ...
        // Read from FIFO
        let zero_read_data = AXI_ST<TEST_0_DATA_W, TEST_0_DATA_W_DIV8, TEST_0_ID_W, TEST_0_DEST_W>
        {
...
// E1219 13:17:55.614045  108138 run_routines.cc:282] Internal error: DEADLINE_EXCEEDED: Exceeded limit of 100000 proc ticks before terminating
// [        FAILED ] test_fifo: internal error: DEADLINE_EXCEEDED: Exceeded limit of 100000 proc ticks before terminating
...
proc test_double_fifo_gpf {
    ch_fifo1_read: chan<AXI_ST<TEST_1_DATA_W, TEST_1_DATA_W_DIV8, TEST_1_ID_W, TEST_1_DEST_W>> in;
    ch_fifo0_write: chan<AXI_ST<TEST_1_DATA_W, TEST_1_DATA_W_DIV8, TEST_1_ID_W, TEST_1_DEST_W>> out;
    terminator: chan<bool> out;

    config(terminator: chan<bool> out) {
        let (ch_fifo0_read_s, ch_fifo0_read_r) =
            chan<AXI_ST<TEST_1_DATA_W, TEST_1_DATA_W_DIV8, TEST_1_ID_W, TEST_1_DEST_W>>;
        ...
        // I find this behaviour confusing and am not sure how to continue with testing/debugging the FIFO
        ...
        spawn gpf::gpf<TEST_1_DATA_W, TEST_1_DATA_W_DIV8, TEST_1_ID_W, TEST_1_DEST_W>(ch_fifo0_read_r,ch_fifo1_write_s );
    ...
    next(tok: token, state: TestState<TEST_1_DATA_W>) {
        // Write to FIFO
        let write_data = AXI_ST<TEST_1_DATA_W, TEST_1_DATA_W_DIV8, TEST_1_ID_W, TEST_1_DEST_W> {
            tdata: state.data,
        ...
        // Read from FIFO
        let zero_read_data = AXI_ST<TEST_1_DATA_W, TEST_1_DATA_W_DIV8, TEST_1_ID_W, TEST_1_DEST_W>
        {
...
// E1219 13:15:11.058317  106946 run_routines.cc:282] Internal error: DEADLINE_EXCEEDED: Exceeded limit of 100000 proc ticks before terminating
// [        FAILED ] test_fifo: internal error: DEADLINE_EXCEEDED: Exceeded limit of 100000 proc ticks before terminating
...
// proc fifo_synth {
//     config( ch_fifo_read:chan<AXI_ST<SYNTH_0_DATA_W, SYNTH_0_DATA_W_DIV8, SYNTH_0_ID_W, SYNTH_0_DEST_W>> out,
//             ch_fifo_write:chan<AXI_ST<SYNTH_0_DATA_W, SYNTH_0_DATA_W_DIV8, SYNTH_0_ID_W, SYNTH_0_DEST_W>> in
//             ) {
//         spawn FIFO<
//             SYNTH_0_DATA_W, SYNTH_0_DATA_W_DIV8, SYNTH_0_ID_W, SYNTH_0_DEST_W, SYNTH_0_ADDR_W, SYNTH_0_FIFO_L>(
