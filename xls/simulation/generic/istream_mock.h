...
class IStreamMock final : public IStream {
 public:
  using Register = RegisterMock<WidthBits>;
  ...
  size_t FifoCapacity() const override { return 4; }

  size_t FifoCount() const override { return fifo_.size(); }

  absl::Status Transfer() override {
    if (IsReadStream_) {
      if (fifo_.empty()) {
        return absl::InternalError("FIFO is empty");
      }
      holding_reg_ = RegisterMock<WidthBits>{fifo_.front()};
      fifo_.pop();
    } else {
