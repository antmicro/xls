...
class SingleValueManager final : public IChannelManager {
 public:
  ...
  absl::Status RegisterChannel(std::unique_ptr<IChannel> reg, uint64_t offset);
  ...
  ~SingleValueManager() override = default;
 ...
 private:
  ...
  absl::StatusOr<IChannel*> GetMappingOrStatus(uint64_t address,
                                               std::string&& err_msg);
  uint64_t GetMaxOffset() const { return this->address_range_end_; }

  std::map<uint64_t, std::unique_ptr<IChannel>> offset_to_channel_;
  std::unordered_map<IChannel*, uint64_t> channel_to_offset_;
};
