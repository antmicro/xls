...
enum FrameHeaderStatus { OK, CORRUPTED, NO_ENOUGH_DATA };
...
class FrameHeaderTest : public xls::IrTestBase {
 public:
  ...
  void ParseAndCompareWithZstd(absl::Span<const uint8_t> buffer) {
    ...
    if (result != 0) {
      if (ZSTD_isError(result)) {
        ...
        // Provided data is to small to correctly parse ZSTD frame, should have
        // `result` bytes, got `buffer.size()` bytes.
        expected_status = FrameHeaderStatus::NO_ENOUGH_DATA;
      }
    } else {
      // Provided data is valid. Successfully parsed ZSTD frame.
      expected_status = FrameHeaderStatus::OK;
    }
  ...
  std::vector<uint8_t> GenerateRandomFrameHeader() {
    ...
    std::uniform_int_distribution<uint8_t> header_size_distribution(0, 32);
    std::uniform_int_distribution<uint8_t> byte_data_distribution(0, 255);

    uint8_t frame_header_size = header_size_distribution(generator);
    std::vector<uint8_t> vec(frame_header_size);
    for (auto& elem : vec) {
      elem = byte_data_distribution(generator);
    }
 ...
 private:
  ...
  Value CreateExpectedBuffer(Value dslx_simulation_input,
                             ...
                             bool expect_failure) {
    if (expect_failure) return dslx_simulation_input;
  ...
  Value CreateExpectedFrameHeaderResult(ZSTD_frameHeader* fh,
    ...
    auto expected_buffer = CreateExpectedBuffer(
        dslx_simulation_input, input_buffer, fh->headerSize,
        (expected_status != FrameHeaderStatus::OK));
...
TEST_F(FrameHeaderTest, ParseFrameHeader) {
  std::vector<uint8_t> buffer{0xE2, 0xFE, 0xCA, 0xEF, 0xCD, 0xAB,
...
TEST_P(FrameHeaderSeededTest, ParseMultipleRandomFrameHeaders) {
  auto frame_header = this->GenerateRandomFrameHeader();
