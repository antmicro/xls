...
class FrameHeaderTest : public xls::IrTestBase {
 public:
  ...
  void ParseAndCompareWithZstd(absl::Span<const uint8_t> buffer) {
    ...
    bool reserved_bit = (input_buffer[0] & (1 << 3)) != 0;
    if (reserved_bit) {
      // Data is corrupted due to reserved bit being set.
      // libzstd first reports NO_ENOUGH_DATA errors. Only when data
      // is long enough for correct parsing it is possible to get CORRUPTED
      // error caused by reserved bit
...
TEST_F(FrameHeaderTest, ParseFrameHeaderFailReservedBit) {
...
TEST_F(FrameHeaderTest, ParseFrameHeaderFailReservedBitNoEnoughData) {
