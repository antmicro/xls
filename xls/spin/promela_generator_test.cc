...
std::string_view BlockBody(std::string_view out, std::string_view header) {
  ...
  if (hpos == std::string_view::npos) return "";
  auto open = out.find('{', hpos);
  if (open == std::string_view::npos) return "";
  int depth = 0;
  for (size_t i = open; i < out.size(); ++i) {
    if (out[i] == '{') ++depth;
    else if (out[i] == '}' && --depth == 0) return out.substr(open, i - open + 1);
  }
  return "";
}
...
// Wraps CompareTraces so callers that use EXPECT_TRUE/EXPECT_FALSE get a
// testing::AssertionResult rather than an absl::Status.
...
testing::AssertionResult DslxPromelaTracesMatchImpl(
  ...
  auto interp_path = GetXlsRunfilePath("xls/dslx/interpreter_main");
  if (!interp_path.ok())
    return testing::AssertionFailure()
           << "interpreter_main: " << interp_path.status();

  auto ir_conv_path =
      GetXlsRunfilePath("xls/dslx/ir_convert/ir_converter_main");
  if (!ir_conv_path.ok())
    return testing::AssertionFailure()
           << "ir_converter_main: " << ir_conv_path.status();

  auto opt_path = GetXlsRunfilePath("xls/tools/opt_main");
  if (!opt_path.ok())
    return testing::AssertionFailure() << "opt_main: " << opt_path.status();

  auto promela_path = GetXlsRunfilePath("xls/spin/promela_main");
  if (!promela_path.ok())
    return testing::AssertionFailure()
           << "promela_main: " << promela_path.status();

  auto tmp = TempDirectory::Create("dslx_promela_trace");
  if (!tmp.ok())
    return testing::AssertionFailure() << "TempDirectory: " << tmp.status();

  const std::filesystem::path dir = tmp->path();
  const std::filesystem::path dslx_trace = dir / "dslx_trace.json";
  const std::filesystem::path ir_file = dir / "out.ir";
  const std::filesystem::path opt_file = dir / "opt.ir";
  const std::filesystem::path pml_file = dir / "out.pml";
  const std::filesystem::path spin_trace = dir / "spin_trace.json";

  std::string dslx_path_flag = "--dslx_path=" + dslx_path_dir.string();

  // Step 1: DSLX interpreter -> text proto trace.
  {
    std::vector<std::string> argv = {
        "/usr/bin/env",         "-u",
        "TESTBRIDGE_TEST_ONLY", "-u",
        "XML_OUTPUT_FILE",      interp_path->string(),
        src_path.string(),      dslx_path_flag,
        "--trace_channels",     "--output_results_proto=" + dslx_trace.string(),
    };
    auto r = InvokeSubprocess(argv);
    if (!r.ok())
      return testing::AssertionFailure()
             << "interpreter_main invoke: " << r.status();
    if (r->exit_status != 0)
      return testing::AssertionFailure()
             << "interpreter_main failed (exit " << r->exit_status << "):\n"
             << r->stderr_content;
  }
  ...
  // Step 2: DSLX -> IR.
  {
    std::vector<std::string> argv = {
        ...
        "--convert_tests=true",
        absl::StrFormat("--top=%s", dslx_top),
        "--output_file=" + ir_file.string(),
    };
    auto r = InvokeSubprocess(argv);
    if (!r.ok())
      return testing::AssertionFailure()
             << "ir_converter_main invoke: " << r.status();
    if (r->exit_status != 0)
      return testing::AssertionFailure()
             << "ir_converter_main failed (exit " << r->exit_status << "):\n"
             << r->stderr_content;
  }

  // Step 3: IR -> optimised IR.
  {
    std::vector<std::string> argv = {
        opt_path->string(),
        ir_file.string(),
        "--output_path=" + opt_file.string(),
    };
    auto r = InvokeSubprocess(argv);
    if (!r.ok())
      return testing::AssertionFailure() << "opt_main invoke: " << r.status();
    if (r->exit_status != 0)
      return testing::AssertionFailure()
             << "opt_main failed (exit " << r->exit_status << "):\n"
             << r->stderr_content;
  }

  // Step 4: optimised IR -> Promela.
  {
    std::vector<std::string> argv = {
        promela_path->string(),
        opt_file.string(),
        "--output=" + pml_file.string(),
    };
    if (emit_termination_hook) argv.push_back("--emit_termination_hook");
    auto r = InvokeSubprocess(argv);
    if (!r.ok())
      return testing::AssertionFailure()
             << "promela_main invoke: " << r.status();
    if (r->exit_status != 0)
      return testing::AssertionFailure()
             << "promela_main failed (exit " << r->exit_status << "):\n"
             << r->stderr_content;
  }

  // Step 5: SPIN guided simulation -> JSON trace.
  {
    std::vector<std::string> argv = {
        spin_path->string(), "-c", "-Q", spin_trace.string(), pml_file.string(),
    };
    auto r = InvokeSubprocess(argv, dir);
    if (!r.ok())
      return testing::AssertionFailure() << "spin invoke: " << r.status();
    if (r->exit_status != 0)
      return testing::AssertionFailure()
             << "spin failed (exit " << r->exit_status << "):\n"
             << r->stderr_content;
  }

  // Step 6: Parse both traces and compare.
  auto spin_json = GetFileContents(spin_trace);
  if (!spin_json.ok())
    return testing::AssertionFailure()
           << "reading spin trace: " << spin_json.status();
  auto dslx_json = GetFileContents(dslx_trace);
  if (!dslx_json.ok())
    return testing::AssertionFailure()
           << "reading dslx trace: " << dslx_json.status();

  std::string_view term_chan = emit_termination_hook ? "terminator" : "";
  TraceMap spin_events, dslx_events;
  if (auto s = ParseSpinTrace(*spin_json, spin_events, term_chan); !s.ok())
    return testing::AssertionFailure() << "parsing spin trace: " << s;
  if (auto s = ParseDslxTrace(*dslx_json, dslx_events, {}, term_chan); !s.ok())
    return testing::AssertionFailure() << "parsing dslx trace: " << s;

  // Save all intermediate artifacts so they survive the temp-dir cleanup.
  // Bazel archives anything written to TEST_UNDECLARED_OUTPUTS_DIR into
  // bazel-testlogs/.../test.outputs/outputs.zip after the test finishes.
  // Use the GTest test name as the subdirectory so parallel tests don't
  // overwrite each other (many tests share the same dslx_top).
  if (const char* out_env = getenv("TEST_UNDECLARED_OUTPUTS_DIR")) {
    const auto* test_info =
        ::testing::UnitTest::GetInstance()->current_test_info();
    std::string subdir =
        test_info ? test_info->name() : std::string(dslx_top);
    std::filesystem::path save_dir =
        std::filesystem::path(out_env) / subdir;
    ...
    for (const auto& [src, name] :
         std::vector<std::pair<std::filesystem::path, std::string>>{
             ...
             {opt_file, "opt.ir"},
             {pml_file, "out.pml"},
             {dslx_trace, "dslx_trace.json"},
             {spin_trace, "spin_trace.json"},
         }) {
  ...
  return CompareMaps(spin_events, dslx_events);
}

// Generates a complete DSLX source with the user function, harness proc
// that receives one set of params, calls fn, sends result and test proc
// that drives the harness with test_cases
std::string GenerateFunctionHarnessDslx(
    std::string_view fn_dslx, std::string_view fn_name,
    const std::vector<DslxFunctionParam>& params, std::string_view return_type,
    const std::vector<std::vector<std::string>>& test_cases) {
  std::string src = "#![feature(type_inference_v2)]\n\n";
  absl::StrAppend(&src, fn_dslx, "\n\n");

  absl::StrAppendFormat(&src, "proc %s_Harness {\n", fn_name);
  for (const auto& p : params)
    absl::StrAppendFormat(&src, "    chan_%s: chan<%s> in;\n", p.name, p.type);
  absl::StrAppendFormat(&src, "    chan_result: chan<%s> out;\n\n",
                        return_type);

  absl::StrAppend(&src, "    config(");
  for (size_t i = 0; i < params.size(); ++i) {
    if (i > 0) absl::StrAppend(&src, ", ");
    absl::StrAppendFormat(&src, "chan_%s: chan<%s> in", params[i].name,
                          params[i].type);
  }
  absl::StrAppendFormat(&src, ", chan_result: chan<%s> out) {\n", return_type);
  absl::StrAppend(&src, "        (");
  for (size_t i = 0; i < params.size(); ++i) {
    if (i > 0) absl::StrAppend(&src, ", ");
    absl::StrAppendFormat(&src, "chan_%s", params[i].name);
  }
  absl::StrAppend(&src, ", chan_result)\n    }\n\n");

  absl::StrAppend(&src, "    init { () }\n\n    next(state: ()) {\n");
  for (size_t i = 0; i < params.size(); ++i) {
    absl::StrAppendFormat(&src, "        let (tok, %s) = recv(%s, chan_%s);\n",
                          params[i].name, (i == 0 ? "join()" : "tok"),
                          params[i].name);
  }
  std::string param_names;
  for (size_t i = 0; i < params.size(); ++i) {
    if (i > 0) absl::StrAppend(&param_names, ", ");
    absl::StrAppend(&param_names, params[i].name);
  }
  absl::StrAppendFormat(&src,
                        "        let r = %s(%s);\n"
                        "        send(tok, chan_result, r);\n"
                        "    }\n}\n\n",
                        fn_name, param_names);

  absl::StrAppendFormat(&src, "#[test_proc]\nproc %s_Test {\n", fn_name);
  for (const auto& p : params)
    absl::StrAppendFormat(&src, "    chan_%s: chan<%s> out;\n", p.name, p.type);
  absl::StrAppendFormat(&src, "    chan_result: chan<%s> in;\n", return_type);
  absl::StrAppend(&src, "    terminator: chan<bool> out;\n\n");

  absl::StrAppend(&src, "    config(terminator: chan<bool> out) {\n");
  for (const auto& p : params)
    absl::StrAppendFormat(
        &src, "        let (chan_%s_s, chan_%s_r) = chan<%s>(\"%s\");\n",
        p.name, p.name, p.type, p.name);
  absl::StrAppendFormat(
      &src,
      "        let (chan_result_s, chan_result_r) = chan<%s>(\"result\");\n",
      return_type);
  absl::StrAppendFormat(&src, "        spawn %s_Harness(", fn_name);
  for (const auto& p : params)
    absl::StrAppendFormat(&src, "chan_%s_r, ", p.name);
  absl::StrAppend(&src, "chan_result_s);\n");
  absl::StrAppend(&src, "        (");
  for (const auto& p : params)
    absl::StrAppendFormat(&src, "chan_%s_s, ", p.name);
  absl::StrAppend(&src, "chan_result_r, terminator)\n    }\n\n");

  // State counter: one test case per next() invocation.
  // Extra iterations after the terminator send are cut by the trace parser.
  absl::StrAppend(&src, "    init { u32:0 }\n\n    next(i: u32) {\n");
  if (test_cases.size() == 1) {
    for (size_t pi = 0; pi < params.size(); ++pi)
      absl::StrAppendFormat(&src, "        let v_%s = %s;\n",
                            params[pi].name, test_cases[0][pi]);
  } else if (params.size() == 1) {
    absl::StrAppendFormat(&src, "        let v_%s: %s = match i {\n",
                          params[0].name, params[0].type);
    for (size_t tc = 0; tc < test_cases.size(); ++tc) {
      if (tc + 1 < test_cases.size())
        absl::StrAppendFormat(&src, "            u32:%zu => %s,\n",
                              tc, test_cases[tc][0]);
      else
        absl::StrAppendFormat(&src, "            _ => %s,\n",
                              test_cases[tc][0]);
    }
    absl::StrAppend(&src, "        };\n");
  } else {
    absl::StrAppend(&src, "        let (");
    for (size_t pi = 0; pi < params.size(); ++pi) {
      if (pi > 0) absl::StrAppend(&src, ", ");
      absl::StrAppendFormat(&src, "v_%s", params[pi].name);
    }
    absl::StrAppend(&src, "): (");
    for (size_t pi = 0; pi < params.size(); ++pi) {
      if (pi > 0) absl::StrAppend(&src, ", ");
      absl::StrAppend(&src, params[pi].type);
    }
    absl::StrAppend(&src, ") = match i {\n");
    for (size_t tc = 0; tc < test_cases.size(); ++tc) {
      if (tc + 1 < test_cases.size())
        absl::StrAppendFormat(&src, "            u32:%zu => (", tc);
      else
        absl::StrAppend(&src, "            _ => (");
      for (size_t pi = 0; pi < params.size(); ++pi) {
        if (pi > 0) absl::StrAppend(&src, ", ");
        absl::StrAppend(&src, test_cases[tc][pi]);
      }
      absl::StrAppend(&src, "),\n");
    }
    absl::StrAppend(&src, "        };\n");
  ...
  for (size_t i = 0; i < params.size(); ++i) {
    absl::StrAppendFormat(
        &src, "        let tok = send(%s, chan_%s, v_%s);\n",
        (i == 0 ? "join()" : "tok"), params[i].name, params[i].name);
  }
  absl::StrAppendFormat(
      &src, "        let (tok, _) = recv(tok, chan_result);\n");
  absl::StrAppendFormat(
      &src, "        send_if(tok, terminator, i == u32:%zu, true);\n",
      test_cases.size() - 1);
  absl::StrAppend(&src, "        i + u32:1\n    }\n}\n");
...
class PromelaGeneratorTest : public IrTestBase {
 protected:
  ...
  testing::AssertionResult DslxFunctionTracesMatch(
      std::string_view fn_dslx, std::string_view fn_name,
      const std::vector<DslxFunctionParam>& params,
...
absl::StatusOr<std::string> PromelaGeneratorTest::Generate(
  ...
  return PromelaGenerator::Generate(pkg.get());
}
...
testing::AssertionResult PromelaGeneratorTest::GenerateAndSpinCheck(
  ...
  auto pml_or = PromelaGenerator::Generate(pkg_or->get());
  if (!pml_or.ok()) {
    return testing::AssertionFailure()
...
testing::AssertionResult PromelaGeneratorTest::DslxPromelaTracesMatch(
  ...
  return DslxPromelaTracesMatchImpl(src, tmp->path(), dslx_top,
...
testing::AssertionResult PromelaGeneratorTest::DslxPromelaTracesMatchFile(
  ...
  return DslxPromelaTracesMatchImpl(*src, src->parent_path(), dslx_top,
                                    emit_termination_hook);
}
...
testing::AssertionResult PromelaGeneratorTest::DslxFunctionTracesMatch(
  ...
  std::string dslx_source = GenerateFunctionHarnessDslx(
      fn_dslx, fn_name, params, return_type, test_cases);
...
TEST_F(PromelaGeneratorTest, InitBlockRunsAllProcs) {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(std::string out,
                           GenerateFromIrFile("multi_proc.ir"));
...
TEST_F(PromelaGeneratorTest, ProcStructure_StateUpdateAfterComputation) {
  ...
  EXPECT_LT(pt.find("v_add_15 = (s___state +"), pt.find("s___state = v_add_15"));
}
...
TEST_F(PromelaGeneratorTest, ProcStructure_XrForRecvXsForSend) {
  // Receive channels get xr, send channels get xs; no cross-assignment.
  XLS_ASSERT_OK_AND_ASSIGN(std::string out, GenerateFromIrFile("proc.ir"));
  std::string_view pt = ProctypeBody(out, "__proc__MyProc_0_next");
  ASSERT_THAT(pt, Not(testing::IsEmpty()));
  EXPECT_THAT(pt, HasSubstr("xr _req_r;"));
  EXPECT_THAT(pt, HasSubstr("xs _resp_s;"));
  EXPECT_THAT(pt, Not(HasSubstr("xs _req_r")));
...
top proc parent<>(__tkn: token, init={token}) {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(std::string out, PromelaGenerator::Generate(pkg.get()));
...
TEST_F(PromelaGeneratorTest, TraceMatch_Neg) {
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: s32) -> s32 { -a }", "op",
                                      {{"a", "s32"}}, "s32",
                                      {{"s32:5"}, {"s32:0"}, {"s32:-3"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_SMul) {
  EXPECT_TRUE(
      DslxFunctionTracesMatch("fn op(a: s32, b: s32) -> s32 { a * b }", "op",
                              {{"a", "s32"}, {"b", "s32"}}, "s32",
                              {{"s32:3", "s32:-4"}, {"s32:-2", "s32:-5"}}));
}

TEST_F(PromelaGeneratorTest, TraceMatch_UDiv) {
  EXPECT_TRUE(
      DslxFunctionTracesMatch("fn op(a: u32, b: u32) -> u32 { a / b }", "op",
                              {{"a", "u32"}, {"b", "u32"}}, "u32",
                              {{"u32:10", "u32:3"}, {"u32:100", "u32:4"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Xor) {
  EXPECT_TRUE(
      DslxFunctionTracesMatch("fn op(a: u32, b: u32) -> u32 { a ^ b }", "op",
                              {{"a", "u32"}, {"b", "u32"}}, "u32",
                              {{"u32:255", "u32:15"}, {"u32:170", "u32:170"}}));
}

TEST_F(PromelaGeneratorTest, TraceMatch_Not) {
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u8) -> u8 { !a }", "op",
                                      {{"a", "u8"}}, "u8",
                                      {{"u8:0"}, {"u8:255"}, {"u8:170"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_AndReduce) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn op(a: u32) -> u1 { and_reduce(!a) }", "op", {{"a", "u32"}}, "u1",
      {{"u32:0"},              // !0 = all_ones -> 1
       {"u32:1"},              // !1 = ...1110  -> 0
...
TEST_F(PromelaGeneratorTest, TraceMatch_OrReduce) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn op(a: u32) -> u1 { or_reduce(a) }", "op", {{"a", "u32"}}, "u1",
      {{"u32:0"},
       {"u32:1"},
       {"u32:1073741824"}}));  // bit 30 set, fits in signed int
}

TEST_F(PromelaGeneratorTest, TraceMatch_Shll) {
  EXPECT_TRUE(
      DslxFunctionTracesMatch("fn op(a: u32, n: u32) -> u32 { a << n }", "op",
                              {{"a", "u32"}, {"n", "u32"}}, "u32",
                              {{"u32:1", "u32:4"}, {"u32:3", "u32:8"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Shrl) {
  EXPECT_TRUE(
      DslxFunctionTracesMatch("fn op(a: u32, n: u32) -> u32 { a >> n }", "op",
                              {{"a", "u32"}, {"n", "u32"}}, "u32",
                              {{"u32:256", "u32:4"}, {"u32:768", "u32:8"}}));
}

TEST_F(PromelaGeneratorTest, TraceMatch_Shra) {
  // Arithmetic right shift on signed type: sign bit is replicated.
  EXPECT_TRUE(
      DslxFunctionTracesMatch("fn op(a: s32, n: u32) -> s32 { a >> n }", "op",
                              {{"a", "s32"}, {"n", "u32"}}, "s32",
                              {{"s32:-256", "u32:4"}, {"s32:256", "u32:4"}}));
}

TEST_F(PromelaGeneratorTest, TraceMatch_Eq) {
  EXPECT_TRUE(
      DslxFunctionTracesMatch("fn op(a: u32, b: u32) -> u1 { a == b }", "op",
                              {{"a", "u32"}, {"b", "u32"}}, "u1",
                              {{"u32:5", "u32:5"}, {"u32:5", "u32:6"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Ne) {
  EXPECT_TRUE(
      DslxFunctionTracesMatch("fn op(a: u32, b: u32) -> u1 { a != b }", "op",
                              {{"a", "u32"}, {"b", "u32"}}, "u1",
                              {{"u32:5", "u32:6"}, {"u32:5", "u32:5"}}));
}

TEST_F(PromelaGeneratorTest, TraceMatch_ULt) {
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn op(a: u32, b: u32) -> u1 { a < b }", "op",
      {{"a", "u32"}, {"b", "u32"}}, "u1",
...
TEST_F(PromelaGeneratorTest, TraceMatch_SGt) {
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: s32, b: s32) -> u1 { a > b }",
                                      "op", {{"a", "s32"}, {"b", "s32"}}, "u1",
                                      {{"s32:0", "s32:-1"},     // 0 > -1 -> 1
                                       {"s32:0", "s32:0"},      // 0 > 0  -> 0
                                       {"s32:-1", "s32:0"}}));  // -1 > 0 -> 0
...
TEST_F(PromelaGeneratorTest, TraceMatch_BitSlice) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn op(a: u16) -> u4 { a[4:8] }", "op", {{"a", "u16"}}, "u4",
...
TEST_F(PromelaGeneratorTest, TraceMatch_DynamicBitSlice) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn op(a: u16, start: u32) -> u4 { a[start +: u4] }", "op",
      {{"a", "u16"}, {"start", "u32"}}, "u4",
...
TEST_F(PromelaGeneratorTest, TraceMatch_SignExtend) {
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn op(a: s8) -> s32 { a as s32 }", "op", {{"a", "s8"}}, "s32",
...
TEST_F(PromelaGeneratorTest, TraceMatch_ZeroExtend) {
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u8) -> u32 { a as u32 }", "op",
                                      {{"a", "u8"}}, "u32",
                                      {{"u8:0"}, {"u8:127"}, {"u8:255"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Concat) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn op(a: u8, b: u8) -> u32 { (a ++ b) as u32 }", "op",
      {{"a", "u8"}, {"b", "u8"}}, "u32", {{"u8:1", "u8:2"}}));  // 0x0102 = 258
}

TEST_F(PromelaGeneratorTest, TraceMatch_Sel) {
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn op(s: u1, a: u32, b: u32) -> u32 { if s == u1:0 { a } else { b } }",
      "op", {{"s", "u1"}, {"a", "u32"}, {"b", "u32"}}, "u32",
...
TEST_F(PromelaGeneratorTest, TraceMatch_OneHotSel) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn op(s: u2, a: u32, b: u32) -> u32 { one_hot_sel(s, [a, b]) }", "op",
      {{"s", "u2"}, {"a", "u32"}, {"b", "u32"}}, "u32",
...
TEST_F(PromelaGeneratorTest, TraceMatch_Gate) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn op(c: u1, a: u32) -> u32 { gate!(c as bool, a) }", "op",
      {{"c", "u1"}, {"a", "u32"}}, "u32",
      {{"u1:1", "u32:42"}, {"u1:0", "u32:42"}}));
}

TEST_F(PromelaGeneratorTest, TraceMatch_Invoke) {
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn helper(x: u32) -> u32 { x + x }\n"
      "fn op(a: u32) -> u32 { helper(a) }",
      "op", {{"a", "u32"}}, "u32", {{"u32:5"}, {"u32:21"}, {"u32:0"}}));
}

TEST_F(PromelaGeneratorTest, TraceMatch_AddMul_U32) {
  // a + b * c: two operations chained, u32 (no overflow).
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn op(a: u32, b: u32, c: u32) -> u32 { a + b * c }", "op",
...
TEST_F(PromelaGeneratorTest, TraceMatch_AddMul_U8_Overflow) {
  ...
  EXPECT_TRUE(
      DslxFunctionTracesMatch("fn op(a: u8, b: u8, c: u8) -> u8 { a + b * c }",
                              "op", {{"a", "u8"}, {"b", "u8"}, {"c", "u8"}},
                              "u8", {{"u8:244", "u8:10", "u8:30"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_CompareAfterOverflow_U8) {
  ...
  // true -> u8:1 (wrong).
  EXPECT_TRUE(
      DslxFunctionTracesMatch("fn op(a: u8, b: u8, t: u8) -> u8 {\n"
                              "  if a + b > t { u8:1 } else { u8:0 }\n"
                              "}",
                              "op", {{"a", "u8"}, {"b", "u8"}, {"t", "u8"}},
                              "u8", {{"u8:200", "u8:100", "u8:100"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Shll_WideValue_NarrowAmount) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u8, n: u4) -> u8 { a << n }",
                                      "op", {{"a", "u8"}, {"n", "u4"}}, "u8",
                                      {{"u8:200", "u4:2"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Shll_NarrowValue_WideAmount) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u4, n: u8) -> u4 { a << n }",
                                      "op", {{"a", "u4"}, {"n", "u8"}}, "u4",
                                      {{"u4:3", "u8:3"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Shrl_WideValue_NarrowAmount) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u12, n: u4) -> u12 { a >> n }",
                                      "op", {{"a", "u12"}, {"n", "u4"}}, "u12",
                                      {{"u12:4095", "u4:4"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Concat_DifferentWidth) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u4, b: u8) -> u12 { a ++ b }",
                                      "op", {{"a", "u4"}, {"b", "u8"}}, "u12",
                                      {{"u4:10", "u8:188"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Concat_ThreeParts) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch(
      "fn op(a: u4, b: u4, c: u8) -> u16 { a ++ b ++ c }", "op",
      {{"a", "u4"}, {"b", "u4"}, {"c", "u8"}}, "u16",
      {{"u4:1", "u4:2", "u8:52"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Add_Overflow_U4) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u4, b: u4) -> u4 { a + b }",
                                      "op", {{"a", "u4"}, {"b", "u4"}}, "u4",
                                      {{"u4:15", "u4:1"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Shll_Overflow_U4) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u4, n: u32) -> u4 { a << n }",
                                      "op", {{"a", "u4"}, {"n", "u32"}}, "u4",
                                      {{"u4:3", "u32:3"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Not_U4) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u4) -> u4 { !a }", "op",
                                      {{"a", "u4"}}, "u4", {{"u4:5"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Add_Overflow_U6) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u6, b: u6) -> u6 { a + b }",
                                      "op", {{"a", "u6"}, {"b", "u6"}}, "u6",
                                      {{"u6:63", "u6:1"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Add_Overflow_U8) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u8, b: u8) -> u8 { a + b }",
                                      "op", {{"a", "u8"}, {"b", "u8"}}, "u8",
                                      {{"u8:255", "u8:1"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Add_Overflow_U12) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u12, b: u12) -> u12 { a + b }",
                                      "op", {{"a", "u12"}, {"b", "u12"}}, "u12",
                                      {{"u12:4095", "u12:1"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Add_Overflow_U20) {
  ...
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u20, b: u20) -> u20 { a + b }",
                                      "op", {{"a", "u20"}, {"b", "u20"}}, "u20",
                                      {{"u20:1048575", "u20:1"}}));
}
...
TEST_F(PromelaGeneratorTest, TraceMatch_Add_Overflow_U31) {
  ...
  // 0x7FFFFFFF = 0
  EXPECT_TRUE(DslxFunctionTracesMatch("fn op(a: u31, b: u31) -> u31 { a + b }",
                                      "op", {{"a", "u31"}, {"b", "u31"}}, "u31",
                                      {{"u31:2147483647", "u31:1"}}));
}
...
TEST_F(PromelaGeneratorTest, ProcAssert) {
  // assert() emits a Promela assert() statement carrying the condition
  // expression.
...
top proc counter(__count: bits[32], init={0}) {
  ...
  EXPECT_THAT(PromelaGenerator::Generate(pkg.get()),
...
fn add(a: bits[32], b: bits[32]) -> bits[32] {
  ...
  XLS_ASSERT_OK_AND_ASSIGN(std::string out, PromelaGenerator::Generate(pkg.get(), opts));
...
TEST_F(PromelaGeneratorTest, SpinSyntax_XrXsHints) {
  EXPECT_TRUE(GenerateAndSpinCheckFromFile("xls/spin/testdata/xr_xs_hints.ir"));
}
