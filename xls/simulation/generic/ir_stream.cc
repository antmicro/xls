...
size_t IRStream::FifoCapacity() const {
  if (this->read_channel_) {
    return std::numeric_limits<size_t>::max();
  }
  return this->kWriteFifoMaxDepth;
}

size_t IRStream::FifoCount() const { return this->ir_stream_queue_->GetSize(); }
...
absl::Status IRStream::Transfer() {
  if (this->read_channel_) {
    ...
    if (!read_value.has_value()) {
      return absl::InternalError(
          absl::StrFormat("Streaming queue: %s was empty during read",
                          this->ir_stream_queue_->channel()->name().data()));
    }
  ...
  if (this->ir_stream_queue_->GetSize() >= this->kWriteFifoMaxDepth) {
    return absl::InternalError(
        absl::StrFormat("Streaming queue: %s overfill",
                        this->ir_stream_queue_->channel()->name().data()));
  }
