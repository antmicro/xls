...
// Mock implementation of a single-value register for tests.
// Exposes normal IChannel interface, as well as test-only
// methods to access underlying data bytes as std::vector
template <uint64_t WidthBits>
class RegisterMock final : public IChannel {
 public:
  ...
  RegisterMock() = default;
  explicit RegisterMock(ByteArray init_bytes) : bytes_{init_bytes} {
