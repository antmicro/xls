...
size_t IrAxiStreamLike::FifoCapacity() const { return kWriteFifoMaxDepth; }

size_t IrAxiStreamLike::FifoCount() const { return queue_->GetSize(); }
...
void IrAxiStreamLike::SetDataValid(uint64_t begin, uint64_t end) {
  XLS_CHECK_LE(begin, num_symbols_);
  XLS_CHECK_LE(end, num_symbols_);
  XLS_CHECK_LE(begin, end);
  for (size_t i = 0; i < num_symbols_; i++) {
    tkeep_reg_[i] = (i >= begin) && (i < end);
  }
}

std::pair<uint64_t, uint64_t> IrAxiStreamLike::GetDataValid() const {
  uint64_t begin, end, past_end;

  for (begin = 0; begin < tkeep_reg_.size(); ++begin) {
    if (tkeep_reg_[begin]) {
      break;
    }
  }
  for (end = begin; end < tkeep_reg_.size(); ++end) {
    if (!tkeep_reg_[end]) {
      break;
    }
  }

  for (past_end = end; past_end < tkeep_reg_.size(); ++past_end) {
    if (tkeep_reg_[past_end]) {
      XLS_LOG(WARNING) << "Non-continuous tkeep vector. Symbols past " << end
