...
// Each channel operation defines node dependency level based on tokens
// it uses. For every `send` node it creates an entry for a post type check pass
// to see if the data can be assembled at given dependency level defined by the
...
class SemanticsAnalysis {
 ...
 private:
  ...
  // Used by kIOOrderingMismatch. It checks whether the data used in `send`
  // channel operation is possible to assemble with constraints of given tokens.
